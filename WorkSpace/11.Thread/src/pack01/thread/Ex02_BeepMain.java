...
public class Ex02_BeepMain {
	public static void main(String[] args) {
		//Ex02_BeepTask eb = new Ex02_BeepTask(); 인스턴스화 : 객체를 메모리에 올려서 (eb)사용할준비함
		Runnable eb = new Ex02_BeepTask();//같은 구조를 가지고있기때문에.(run)
		// 객체를 생성할때 쓰이는 메소드 : 생성자 메소드 
		// 생성자 메소드는 클래스의 이름과 똑같음.
		// 메소드이기때문에 오버로딩이 가능하다. ( 같은 이름으로 타입이나 , 파라메터 개수를 바꿔서 중첩시키는것)
